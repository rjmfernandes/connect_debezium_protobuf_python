...
# Function to fetch Protobuf schema from Schema Registry
def fetch_protobuf_schema(schema_id):
    url = f"http://localhost:8081/schemas/ids/{schema_id}"
    response = requests.get(url)
    response.raise_for_status()
    return response.json()['schema']

# Function to create a Protobuf message type dynamically
...
# Example schema ID to fetch
schema_id = 1  # Change this as needed

# Fetch the Protobuf schema dynamically
schema_text = fetch_protobuf_schema(schema_id)

# Create the Protobuf message type dynamically
message_class = create_protobuf_message_type(schema_text)

# Initialize ProtobufDeserializer with the dynamically created message type
...
try:
    while True:
        ...
        deserialized_data = protobuf_deserializer(
